macro_rules! assert_chapter {
    ($title:expr, $chapter:expr, $expected:expr) => {
        assert_eq!(crate::parse_chapter_number($title, $chapter), $expected)
    };
}
...
fn basic_ch_prefix_with_decimal() {
    ...
    assert_chapter!(
        manga_title,
        "Mokushiroku Alice Vol.1 Ch.4.1: Misrepresentation",
        4.1
    );
    assert_chapter!(
        manga_title,
...
fn basic_ch_prefix_with_alpha_postfix() {
    ...
    assert_chapter!(
        manga_title,
        "Mokushiroku Alice Vol.1 Ch.4.a: Misrepresentation",
        4.1
    );
    assert_chapter!(
        manga_title,
        "Mokushiroku Alice Vol.1 Ch.4.b: Misrepresentation",
        4.2
    );
    assert_chapter!(
        manga_title,
        "Mokushiroku Alice Vol.1 Ch.4.extra: Misrepresentation",
        4.99
    );
}
...
fn chapter_title_with_ch_substring() {
    ...
    assert_chapter!(
        manga_title,
        "Vol.1 Ch.2: March 25 (First Day Cohabiting)",
        2.0
    );
}
...
fn chapter_title_containing_commas() {
    ...
    assert_chapter!(
        manga_title,
        "One Piece the sunny, goes swimming 024,005",
        24.005
    );
}
